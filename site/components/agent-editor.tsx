                    ...
                    <motion.div key="yaml" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                        <div className="relative">
                            ...
                            <div className="flex rounded-xl overflow-hidden border border-border-accent/70 shadow-[0_0_30px_rgba(15,240,146,0.04)]">
                                {/* Gutter */}
                                <div className="py-4 px-3 bg-black/60 select-none border-r border-border-accent/30 min-w-[2.5rem] text-right">
                                    {yamlStr.split("\n").map((_, idx) => (
                                        <div key={idx} className="text-slate-600 text-[11px] font-mono leading-[1.625rem]">
                                            {idx + 1}
                                        </div>
                                    ))}
                                </div>
                    ...
                    <motion.div key="visual" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-8">
                        ...
                        <section>
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <h2 className="text-slate-100 text-xl font-bold tracking-tight font-display">I/O Ports</h2>
                                <span className="text-xs text-slate-500 ml-auto">Conecta agentes em pipeline</span>
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="space-y-2">
                                    ...
                                    {(agent.io?.inputs || []).map((inp, idx) => (
                                        <div key={idx} className="bg-slate-panel border border-border-accent/40 rounded-lg p-3 space-y-2 relative">
                                            <button onClick={() => {
                                            ...
                                            <input value={inp.name} onChange={e => {
                                                ...
                                                setAgentField({ io: { ...agent.io, inputs: next } });
                                            }} className="form-input w-full rounded border-border-accent bg-black/30 text-slate-100 h-8 px-2 text-xs placeholder:text-slate-600" placeholder="Input name" />
                                            <input value={(inp.accepts || []).join(", ")} onChange={e => {
                                                const next = [...(agent.io?.inputs || [])];
                                                next[idx] = { ...next[idx], accepts: e.target.value.split(",").map(s => s.trim()).filter(Boolean) };
                                                setAgentField({ io: { ...agent.io, inputs: next } });
                                            }} className="form-input w-full rounded border-border-accent bg-black/30 text-slate-100 h-8 px-2 text-xs placeholder:text-slate-600" placeholder="accepts: text/plain, text/markdown" />
                                        </div>
                                    ))}
                                    <button onClick={() => setAgentField({ io: { ...agent.io, inputs: [...(agent.io?.inputs || []), { name: "context", accepts: ["text/plain"] }] } })}
                                        className="w-full flex items-center justify-center gap-1 h-8 rounded border border-dashed border-border-accent/50 text-slate-500 hover:text-primary hover:border-primary/50 text-xs transition-colors">
                                    ...
                                    {(agent.io?.outputs || []).map((out, idx) => (
                                        <div key={idx} className="bg-slate-panel border border-border-accent/40 rounded-lg p-3 space-y-2 relative">
                                            ...
                                            <input value={out.name} onChange={e => {
                                                ...
                                                next[idx] = { ...next[idx], name: e.target.value };
                                                setAgentField({ io: { ...agent.io, outputs: next } });
                                            }} className="form-input w-full rounded border-border-accent bg-black/30 text-slate-100 h-8 px-2 text-xs placeholder:text-slate-600" placeholder="Output name" />
                                            <input value={(out.produces || []).join(", ")} onChange={e => {
                                                const next = [...(agent.io?.outputs || [])];
                                                next[idx] = { ...next[idx], produces: e.target.value.split(",").map(s => s.trim()).filter(Boolean) };
                                                setAgentField({ io: { ...agent.io, outputs: next } });
                                            }} className="form-input w-full rounded border-border-accent bg-black/30 text-slate-100 h-8 px-2 text-xs placeholder:text-slate-600" placeholder="produces: text/markdown" />
                                        </div>
                                    ))}
                        ...
                        <section>
                            <div className="flex items-center gap-2 mb-3">
                                ...
                                <h2 className="text-slate-100 text-xl font-bold tracking-tight font-display">Metadata</h2>
                            </div>
                            <div className="space-y-2">
                                {Object.entries(agent.metadata || {}).map(([k, v]) => (
                                    <div key={k} className="flex gap-2 items-center">
                                        <input value={k} readOnly className="form-input w-1/3 rounded border-border-accent bg-slate-panel text-slate-300 h-9 px-3 text-xs" />
                                        <span className="text-slate-600">:</span>
                                        <input value={v} onChange={e => {
                                            const next = { ...(agent.metadata || {}), [k]: e.target.value };
                                            setAgentField({ metadata: next });
                                        }} className="form-input flex-1 rounded border-border-accent bg-slate-panel text-slate-100 h-9 px-3 text-xs" />
                                        <button onClick={() => {
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {filteredSkills.map(skill => {
                                    ...
                                    return (
                                        ...
                                        }} className={`flex items-center gap-3 p-3 rounded cursor-pointer transition-all duration-200 border ${isSelected ? "bg-primary/20 border-primary" : "border-border-accent/50 bg-slate-panel/40 hover:bg-slate-panel/80"}`}>
