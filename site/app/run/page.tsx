...
import { fetchAgents, runTeam, type Agent, type RunMode } from "@/lib/api";
import { Play, Square, Users, ArrowRight, CheckSquare, Square as UncheckedSquare, Layers } from "lucide-react";
...
export default function RunPage() {
    ...
    function handleRun() {
        ...
        abortRef.current = runTeam(selected, mode, task, {
            onAgentStart: ({ agent }) => {
                setResults(prev => [...prev, { agent, text: "", running: true }]);
            },
            onAgentResult: ({ agent, text }) => {
                setResults(prev => {
                    let idx = -1;
    ...
    const canRun = selected.length > 0 && task.trim() !== "" && !running;
                        ...
                        {(["sequential", "parallel"] as RunMode[]).map(m => (
                            ...
                            >
                                ...
                                <div className="text-xs font-normal opacity-70 mt-0.5">
                                    {m === "sequential" ? "Saída de um alimenta o próximo" : "Todos ao mesmo tempo"}
                                </div>
                            </button>
                        ))}
                    </div>
